...
def hf_hub_download(key, filename):
    if Path(key).is_dir():
        return str(Path(key) / filename)
    return _hub_download(key, filename)
...
def _check_key(key: str, part: str):
    if Path(key).is_dir() and key not in _MODELS:
        _MODELS[key] = dict(_MODELS[_DEFAULT_MODEL])
...
def load_unet(key: str = _DEFAULT_MODEL, float16: bool = False):
    ...
    model = UNetModel(
        UNetConfig(
            ...
            transformer_layers_per_block=([config.get("transformer_layers_per_block", 1)] * n_blocks if isinstance(config.get("transformer_layers_per_block", 1), int) else config["transformer_layers_per_block"]),
            num_attention_heads=(
